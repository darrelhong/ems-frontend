...
export default function BpPaymentView({
  clientSecret,
  attendee,
  // partner,
  checkoutResponse,
  ...
  const handleSubmit = async (ev) => {
    ...
    const applicationId = checkoutResponse.sellerApplication.id;
    ...
    if (payload.error) {
      ...
      setProcessing(false);
      setSucceeded(true);
      //endpoint for completed
      await api
        .post(`/api/sellerProfile/payment-complete/${applicationId}`)
