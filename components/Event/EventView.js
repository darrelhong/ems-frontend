...
const EventView = ({ events, layout }) => {
  ...
  const deleteCancelEvent = async (event) => {
    ...
    if (
      ...
      await handleDelete(event).then((output) => {
        console.log(output);
        output
          ? createToast('Event has been successfully deleted', 'success')
          : CreateToast(
              'An Error occured while trying to delete this event',
              'warning'
            );
      });
    } else {
      await handleCancel(event).then((output) => {
        console.log(output);
        output === false
          ? CreateToast(
              'An Error occured while trying to cancel this event',
              'warning'
            )
