...
export async function getEventsThisWeekend(pageParam = 0, isPublic = false) {
  if (pageParam == 0) {
    ...
    if (isPublic) {
      const { data } = await api.get(`/api/event/public/getEventsThisWeekend/${pageParam}`);
      return data;
    }
    else {
...
export async function getEventsNextWeek(pageParam = 0, isPublic = false) {
  if (pageParam == 0) {
    ...
    if (isPublic) {
      const { data } = await api.get(`/api/event/public/getEventsNextWeek/${pageParam}`);
      return data;
    }
    else {
...
export async function getTopTenEvents(isPublic = false) {
  if (isPublic){
    const { data } = await api.get(`/api/event/public/getTopTenEvents`);
    return data;
  }
  else {
