...
export async function createSellerApplication(
  ...
  const { data } = await api.post(
    `/api/sellerProfile/create-application?eid=${eventId}&id=${bpId}`,
    applicationDetails
  );
  return data;
}
...
export function returnNewSellerApplications(sellerApplications) {
  return sellerApplications.filter(
    (sellerApplication) =>
      sellerApplication.sellerApplicationStatus == 'PENDING'
  );
}
...
export async function getSellerProfileIdFromApplication(id) {
  try {
    const { data } = await api.get(
      `/api/sellerApplication/get-sellerprofile-id/${id}`
    );
    return data;
  } catch (e) {
    console.log(e);
...
export async function getOrganiserFromApplication(id) {
  const { data } = await api.get(`/api/sellerApplication/get-organiser/${id}`);
  return data;
}
